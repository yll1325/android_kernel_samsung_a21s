...
#define CMD_SET_BSS_CHANNEL_WIDTH "CMD_SET_BSS_CHANNEL_WIDTH"
...
int slsi_set_bss_bandwidth(struct net_device *dev, char *command, int cmd_len)
{
	...
	if (bandwidth != 20 && bandwidth != 40 && bandwidth != 80) {
		SLSI_ERR(sdev, "Invalid bandwidth: '%s'\n", ioctl_args->args[0]);
		kfree(ioctl_args);
		return -EINVAL;
	}
	...
	if (ndev_vif->vif_type != FAPI_VIFTYPE_STATION ||
	   (ndev_vif->vif_type == FAPI_VIFTYPE_STATION && ndev_vif->sta.vif_status != SLSI_VIF_STATUS_CONNECTED)) {
		SLSI_NET_ERR(dev, "Must be STA vif and connected\n");
		SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
		kfree(ioctl_args);
		return -EPERM;
	}

	SLSI_NET_DBG1(dev, SLSI_MLME, "mlme_set_bss_max_channel_width_req(vif:%u, bw:%d)\n", ndev_vif->ifnum, bandwidth);
	...
	fapi_set_u32(req, u.mlme_set_bss_max_channel_width_req.spare_1, bandwidth);
...
#define MAX_LEN_PRIV_COMMAND    4096 /*This value is the max reply size set in supplicant*/
	...
	if (strncasecmp(command, CMD_SETSUSPENDMODE, strlen(CMD_SETSUSPENDMODE)) == 0) {
		...
		/* Changing the BW in regulatory database*/
		ret = slsi_set_bandwidth(dev, command, priv_cmd.total_len);
	} else if (strncasecmp(command, CMD_SET_BSS_CHANNEL_WIDTH, strlen(CMD_SET_BSS_CHANNEL_WIDTH)) == 0) {
		/* Changing the Tx BW only, without notifying the AP */
		ret = slsi_set_bss_bandwidth(dev, command, priv_cmd.total_len);
#ifdef CONFIG_SCSC_WLAN_WIFI_SHARING
#if !defined(CONFIG_SCSC_WLAN_MHS_STATIC_INTERFACE) || (defined(SCSC_SEP_VERSION) && SCSC_SEP_VERSION < 90000)
