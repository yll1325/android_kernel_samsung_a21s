...
void slsi_nan_dump_vif_data(struct slsi_dev *sdev, struct netdev_vif *ndev_vif)
{
	...
	for (i = 0; i < SLSI_NAN_MAX_NDP_INSTANCES; i++) {
		...
		SLSI_INFO(sdev, "ndp_ndi: %pM, ndp_instance_id2ndl_vif: %d\n", nan.ndp_ndi[i], nan.ndp_instance_id2ndl_vif[i]);
...
void slsi_nan_ndp_setup_ind(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb, bool is_req_ind)
{
	...
	if (ndp_setup_response == NAN_DP_REQUEST_ACCEPT) {
		...
		if (ndp_instance_id == 0 || ndp_instance_id > SLSI_NAN_MAX_NDP_INSTANCES + 1) {
		...
		if (!data_dev) {
			SLSI_ERR(sdev, "no data_dev for ndp_instance_id:%d ndi[%pM]\n", ndp_instance_id, ndev_vif->nan.ndp_ndi[ndp_instance_id - 1]);
			goto exit;
		}
		ndev_data_vif = netdev_priv(data_dev);
		SLSI_MUTEX_LOCK(ndev_data_vif->vif_mutex);
		...
		peer = slsi_get_peer_from_mac(sdev, dev, peer_ndi);
		if (peer) {
			peer->ndp_count++;
		} else {
...
void slsi_nan_del_peer(struct slsi_dev *sdev, struct net_device *dev, u8 *local_ndi, u16 ndp_instance_id)
{
	...
	if (peer) {
		...
		SLSI_ERR(sdev, "no peer for ndp_instance_id:%d ndi[%pM]\n", ndp_instance_id, ndev_vif->nan.ndp_ndi[ndp_instance_id - 1]);
	}
	SLSI_MUTEX_UNLOCK(ndev_data_vif->vif_mutex);
}

void slsi_nan_ndp_termination_handler(struct slsi_dev *sdev, struct net_device *dev, u16 ndp_instance_id, u16 ndl_vif, u8 *ndi)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct sk_buff *nl_skb;
	struct netdev_vif *ndev_data_vif = NULL;
...
{
	u16 ndl_vif_id, ndp_instance_id;
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	...
	if (ndp_instance_id <= SLSI_NAN_MAX_NDP_INSTANCES)
		slsi_nan_ndp_termination_handler(sdev, dev, ndp_instance_id, ndl_vif_id, ndev_vif->nan.ndp_ndi[ndp_instance_id - 1]);
